...
This section includes descriptions of the following events related to
certificates, which are awarded to qualified learners when they complete a
course.
...
.. list-table::
   ...
   * - ``generation_mode``
     ...
     - Indicates whether this certificate was generated for all learners in a
       course as a batch command, or whether a learner generated her own
...
``edx.certificate.shared`` as they do for ``edx.certificate.created``.
...
.. list-table::
   ...
   * - ``source_url``
     ...
     - The URL of the web site where the certificate evidence link was
       clicked. This URL is the same as the URI in the ``context.referer``
       field. For details, see `context.referer <http://edx.readthedocs.org/projects/devdata/en/latest/internal_data_formats/tracking_logs.html#referer-field>`_.
...
Instructor events that have additional ``context`` or ``event`` member fields
follow.
